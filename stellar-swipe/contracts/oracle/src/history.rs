...
pub fn store_price(env: &Env, pair: &AssetPair, price: i128) {
    ...
    env.storage().persistent().extend_ttl(&key, DAY_IN_LEDGERS * 7, DAY_IN_LEDGERS * 7);
...
pub fn calculate_twap(env: &Env, pair: &AssetPair, window_seconds: u64) -> Result<i128, OracleError> {
...
pub fn get_twap_deviation(env: &Env, pair: &AssetPair, current_price: i128, window: u64) -> Result<i128, OracleError> {
...
mod tests {
    ...
    fn test_pair(env: &Env) -> AssetPair {
        AssetPair {
            base: Asset { code: String::from_str(env, "USDC"), issuer: Some(Address::generate(env)) },
            quote: Asset { code: String::from_str(env, "XLM"), issuer: None },
        }
    }
